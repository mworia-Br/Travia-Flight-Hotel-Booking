...
function handleFromLocation() {
  ...
  if (fromInput.length > 1) {
    fetch(`https://traviabooking.azurewebsites.net/booking/select_destination/${fromInput}`)
...
function handleToLocation() {
  ...
  if (toInput.length > 1) {
    fetch(`https://traviabooking.azurewebsites.net/booking/select_destination/${toInput}`)
      .then((response) => response.json())
...
function handleFindFlight() {
  ...
  fetch(
    `https://traviabooking.azurewebsites.net/booking/search_offers/?originCode=${originCode}&destinationCode=${destinationCode}&departureDate=${departureDate}`
  )
    .then((response) => response.json())
    .then((data) => {
      flights = data.data;
...
function BookFlight(flight) {
  ...
  fetch("https://traviabooking.azurewebsites.net/booking/price_offers", {
    method: "POST",
    headers: {
    ...
    .then((dataObject) => {
      ...
      fetch("https://traviabooking.azurewebsites.net/booking/book_flight/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
