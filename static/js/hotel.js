...
function handleLocation() {
  ...
  if (locationInput && locationData) {
    ...
    if (locationInputValue.length > 1) {
        fetch(`https://${checkoutHost}/api/v1/flight/select_destination/${locationInputValue}`)
...
function handleFindHotel() {
  ...
  const hotelresults_URL = `https://${checkoutHost}/api/v1/hotel/search_hotels/?locationCode=${locationCode}&checkInDate=${checkInDate}&checkOutDate=${checkOutDate}`;
...
function handleFindHotelstopped() {
  ...
  fetch(`https://${checkoutHost}/api/v1/flight/search_hotels/?locationCode=${locationCode}&checkInDate=${checkInDate}&checkOutDate=${checkOutDate}&rooms=${rooms}&adultsPerRoom=${adultsPerRoom}&childrenPerRoom=${childrenPerRoom}`)
    .then((response) => response.json())
    .then((data) => {
      hotels = data.data;
      console.log(hotels);
